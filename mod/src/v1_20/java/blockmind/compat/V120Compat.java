...
/**
 * MinecraftCompat implementation for MC 1.20.x using Yarn mappings.
 *
 * Source set: src/v1_20/java — only compiled when targeting MC 1.20.x
 */
public class V120Compat implements MinecraftCompat {

    private static final Logger LOGGER = LoggerFactory.getLogger("blockmind.compat.v120");
    ...
    public Object createPlayer(Object server, Object world, GameProfile profile) {
        ...
        try {
            // Try 4-parameter constructor first (MC 1.20.2+ with SyncedClientOptions)
            ...
            try {
                // Fallback: 3-parameter constructor (MC 1.20.0-1.20.1)
                Constructor<?> ctor = ServerPlayerEntity.class.getConstructor(
                        MinecraftServer.class, ServerWorld.class, GameProfile.class);
                return ctor.newInstance(mcServer, mcWorld, profile);
            } catch (Exception e2) {
                throw new RuntimeException("Failed to create ServerPlayerEntity for MC 1.20.x", e2);
            }
        }
    }
    ...
    public void registerChatListener(BiConsumer<String, String> handler) {
        ...
        LOGGER.info("[BlockMind] Chat event listener registered (V120Compat)");
    }
    ...
    public boolean isBotPlayer(Object player) {
        // V120Compat creates plain ServerPlayerEntity; BotPlayer wrapper is external
        return false;
    }
}
